...
class Repository(object):
    ...
    def __resource_name_working_path(self, name):
        return os.path.join(self.working, str(os.getpid()), name)
    ...
    def __file_working_path(self, resource_file):
        return os.path.join(self.working, str(os.getpid()), resource_file.location_or_remote())

    def __cache_expired(self, local_path):
        if os.path.exists(local_path):
            if (time.time() - os.stat(local_path)[stat.ST_MTIME]) > self.stale_time:
                return true
        return false
    ...
    def __download(self, key_name, dest_path):
        ...
        if local_exists and (time.time() - os.stat(dest_path)[stat.ST_MTIME]) < self.stale_time:
    ...
    def __move_local(self, src_path, dest_path):
        makedirs(os.path.dirname(dest_path))
        logger.debug("Copying file %s to destination path %s", src_path, dest_path)
    ...
    def __save_resource_file(self, resource_file):
        ...
        if resource_file.path and os.path.exists(resource_file.path) and resource_file.location():
            if resource_file.path != self.local_cache:
                self.__move_local(resource_file.path, file_cache_path)
                resource_file.path = file_cache_path
            if self.bucket:
    ...
    def edit_resource(self, resource):
        ...
        for resource_file in resource.files:
            working_path = self.__file_working_path(resource_file)
            makedirs(os.path.dirname(working_path))
            resource_file.relocate(working_path)
        resource.edit = True

    def save(self, resource):
        if not resource.edit:
            raise ValueError("Resource is not currently being edited")

        for resource_file in resource.files:
            self.__save_resource_file(resource_file)

        # Save encoded JSON version of resource into file in local cache
        resource_cache_path = self.__resource_name_cache_path(resource.name)
        logger.debug("Ensuring resource cache path %s exists", resource_cache_path)
        makedirs(os.path.dirname(resource_cache_path))
        with open(resource_cache_path, 'w') as fh:
            logger.debug("Writing JSON serialised resource to %s", resource_cache_path)
            fh.write(ResourceJSONEncoder().encode(resource))
        if self.bucket:
...
class Resource(object):
    def __init__(self, name, files=None, **kwargs):
        """
        Create a Resource object of the given name with the provided array of 
        File objects and keyword metadata.
        """
        ...
        if files_data:
            ...
            for file_data in files_data:
                ...
                if remote:
                    ...
                    meta['location'] = os.path.join('files', name, os.path.basename(path))
                    if path:
    ...
    def local_paths(self):
        ...
        (Note that this method will trigger a refresh of the Resource, ensuring 
        that all locally-stored data is relatively up-to-date.)
        """
        if self.repository:
...
class ResourceFile(object):
    ...
    def relocate(self, dest_path):
        if self.path:
            shutils.move(self.path, dest_path)
            self.path = dest_path
